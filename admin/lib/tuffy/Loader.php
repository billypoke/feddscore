...
class Tuffy_Loader {
    ...
    public function loadClass ($className) {
        ...
        for ($i = count($parts); $i > 0; $i--) {
            ...
            if (is_file($filename)) {
                tuffyDebug("Loading Class", $className . " from " .
                           $filename, 0, 3);
                require($filename);
...
class Tuffy_Loader_Configuring extends Tuffy_Loader {
     ...
     * implementation ALWAYS configures the module. You may wish to override
     * this.
     *
...
class Tuffy_Loader_ForTuffy extends Tuffy_Loader_Configuring {
    /**
     * Decides whether to configure the module or not based on the value of
     * the `APP_CONFIGURE_MODULE_NAME_WITHOUT_TUFFY_IN_FRONT` constant.
     *
     * @param string The module to check the settings for.
     */
    public function shouldConfigure ($module) {
        $const = 'APP_CONFIGURE_' . strtoupper(substr($module, 6));
